...
계좌해지 요청을 수령해 Source에 명시된 서류 구성·확인·은행 신청·잔액 처리 확인·결과 수령·저장·전달 단계를 수행하고 완료 상태를 관찰 가능하게 기록한다.

## 시작·종료 범위

- 시작: 지원팀 담당자 또는 원문 지정 Actor가 계좌해지 요청을 수령한 상태
- 종료: 결과 스캔·저장 단계와 담당 관리역 전달·완료 처리 단계의 결과가 확인된 상태
- 제외: 미결제 거래 확인, 해지 전 잔액 조건, 수탁계좌 공통 규칙, 폐업·청산과의 확정 순서, 통장·OTP 후속 처리 기준

## Trigger

- 계좌해지 요청 수령 — CONFIRMED (`N-05-09/5.9/01`)
...
Source는 `지원팀 담당자 또는 원문 지정 Actor`와 최종 전달 대상인 담당 관리역만 식별한다. 아래 책임 배분 전체는 `PROVISIONAL`이며, Source로 확정되지 않은 담당 관리역·GP·은행의 세부 책임은 표 안에서 `UNKNOWN`으로 유지한다. 은행은 내부 책임자가 아닌 외부 참여자이고, 각 Activity의 `A`는 한 명으로 제한한다.

| Activity | 지원팀 담당자 또는 원문 지정 Actor | 담당 관리역 | GP | 은행 |
|---|---|---|---|---|
| 해지 요청 수령 | A/R | UNKNOWN | UNKNOWN | - |
| 구비서류 구성 | A/R | UNKNOWN | UNKNOWN | - |
| 해지서류 기재·날인 확인 | A/R | UNKNOWN | UNKNOWN | - |
| 은행 방문·해지 신청 | A/R | UNKNOWN | - | C |
| 잔액 이체·수수료 확인 | A/R | UNKNOWN | - | C |
| 거래내역 확인증 수령 | A/R | UNKNOWN | - | C |
| 결과 스캔·저장 | A/R | UNKNOWN | - | - |
| 관리역 전달·완료 처리 | A/R | I | - | - |
...
|---|---|---|---|---|---|---|---|
| AC-01 | 지원팀 담당자 또는 원문 지정 Actor | 계좌해지 요청을 수령한다. | 계좌해지 요청 | 요청 수령 상태 | N-05-09/5.9/01 | 해당 해지 건의 요청 수령 여부가 기록되어 있다. | CONFIRMED |
| AC-02 | 지원팀 담당자 또는 원문 지정 Actor | 구비서류를 구성한다. | 해지 요청, 확인된 서류 요구 | 구비서류 세트 | N-05-09/5.9/02 | 확인된 요구에 대응하는 문서 목록과 각 문서의 준비 여부가 기록되어 있다. | CONFIRMED |
| AC-03 | 지원팀 담당자 또는 원문 지정 Actor | 해지서류의 기재 및 날인 상태를 확인한다. | 해지서류 | 기재·날인 확인 결과 | N-05-09/5.9/03 | 확인 대상별 기재·날인 상태 또는 `UNKNOWN`이 기록되어 있다. | PROVISIONAL |
| AC-04 | 지원팀 담당자 또는 원문 지정 Actor | 은행을 방문해 계좌해지를 신청한다. | 확인된 제출 서류 | 해지 신청 상태 | N-05-09/5.9/04 | 은행 방문과 해지 신청의 접수 여부가 기록되어 있다. | CONFIRMED |
| AC-05 | 지원팀 담당자 또는 원문 지정 Actor | 잔액 이체 및 수수료를 확인한다. | 은행이 제시한 잔액 처리·수수료 정보 | 잔액 이체·수수료 확인 결과 | N-05-09/5.9/05 | 이체 처리 여부와 수수료 확인 결과 또는 `UNKNOWN`이 각각 기록되어 있다. | PROVISIONAL |
| AC-06 | 지원팀 담당자 또는 원문 지정 Actor | 거래내역 확인증을 수령한다. | 해지 처리 상태 | 거래내역 확인증 수령 상태 | N-05-09/5.9/06 | 해당 해지 건의 거래내역 확인증 수령 여부가 기록되어 있다. | PROVISIONAL |
| AC-07 | 지원팀 담당자 또는 원문 지정 Actor | 결과를 스캔하고 저장한다. | 해지 결과 문서 | 결과 스캔·저장 결과 | N-05-09/5.9/07 | 스캔 및 저장 단계의 결과가 확인된다. | CONFIRMED |
| AC-08 | 지원팀 담당자 또는 원문 지정 Actor | 결과를 담당 관리역에게 전달하고 완료 처리한다. | 해지 결과, 저장 단계 결과 | 관리역 전달·완료 처리 결과 | N-05-09/5.9/08 | 담당 관리역 전달 및 완료 처리 단계의 결과가 확인된다. | CONFIRMED |

### Source Coverage Summary

| Source/context | In-scope units | Covered by | Coverage | Use and limitation |
|---|---:|---|---:|---|
| N-05-09 | 8 atomic tasks (`5.9/01~08`) | AC-01~AC-08 | 8/8 (100%) | 실제 단계·분기·예외·결과 전달의 주 근거 |
...
- R-1 (`공통`, CONFIRMED): N-05-09의 8개 원문 단계를 본선으로 유지한다. (`N-05-09/5.9/01~08`)
...
- 사용한 CASE 없음.
- CASE는 증거로만 사용하며 단일 CASE로 공통 Rule이나 해지 순서를 확정하지 않는다.

## 상태

- CONFIRMED: N-05-09의 AC-01, AC-02, AC-04, AC-07, AC-08과 N-06/6.9의 정의 연결
- PROVISIONAL: AC-03, AC-05, AC-06, 예외 분기의 조건·Actor·복귀점, N-08/E2E-09의 사후 업무 상태
