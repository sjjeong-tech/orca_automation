...
- R-6 (`조건부`, PROVISIONAL): 일반계좌·안전계좌·수탁·기타 계좌 요청 경로의 존재만 기록하며 Actor·시스템·선택 Rule은 확정하지 않는다. (`N-05-10` Claude Finding Enrichment)
- R-7 (`조건부`, PROVISIONAL): BC-07의 수령 채널 후보로 스캔본·팩스본을 기록하되 채널 선택과 수신 완료 기준은 `UNKNOWN`이다.
- R-8 (`조건부`, PROVISIONAL): 스캔본·팩스본 수령 후 원본 수령·보관은 후속 후보로 분리한다.
- R-9 (`조건부`, UNKNOWN): 원본 후속의 필요 여부, Actor, 저장·보관 위치와 완료조건은 확정하지 않는다.
...
- 스캔본·팩스본 수령 채널 후보 (`N-05-10` Claude Finding Enrichment, PROVISIONAL)
...
- 스캔본·팩스본 수령 후 원본 수령·반환·보관은 `PROVISIONAL` 후보다.
- 원본 필요 여부, Actor와 완료 기준은 `UNKNOWN`이다.
...
- 스캔본·팩스본 채널과 유형별 요청 경로의 존재는 `PROVISIONAL`이다.
- 채널 선택, 저장 위치, 공유 수신자, 원본 전달·반환·보관 방식은 `UNKNOWN`이다.
...
- PROVISIONAL: 필요·발급정보·사전 제출·추가서류 여부 확인, 일반·안전·수탁·기타 계좌 요청 경로, 스캔본·팩스본 수령, 원본 후속 후보, RACI, Decision·Exception·Automation Candidate
- CONFLICT: 현재 확인된 Source 간 충돌 없음
- UNKNOWN: 요청 주체·일자, 계좌 유형별 Actor·시스템·선택 Rule, 대상 계좌·은행, 발급 방식, 채널 선택 기준, 수수료·위임장·긴급도·부수·지점 차이, 공유 수신자, 원본 Actor·완료조건, 모든 Interface
