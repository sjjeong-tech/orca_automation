...
| UC-02 | 지원팀 담당자 또는 원문 지정 Actor | 정정 사유와 서류를 확인한다. | 수령한 요청, 제시된 서류 | 확인 결과 | N-05-05/5.5/02 | 정정 사유와 서류의 확인 결과가 기록되고, 보강된 사유 카테고리는 `PROVISIONAL`, 미분류 사유는 `UNKNOWN`으로 남는다. | CONFIRMED |
...
- R-5 (`조건부`, PROVISIONAL): 조합명·소재지·대표자·GP·공동 GP·등록 오류가 정정 사유 카테고리로 제시된다. 완전한 폐쇄 목록, 빈도·우선순위·자동 선택 기준은 `UNKNOWN`이다. (`N-05-05` Claude Finding Enrichment)
...
| UC-D03 | UNKNOWN | UC-08 | 기관 요구 | 보완인가 취하인가 | 보완: 기존 접수 유지 또는 재접수 여부 판단 | 취하: 취하 후 재접수 여부 판단 | 감지조건, 판단 Actor, 복귀·종료조건, 자동 선택 기준 | PROVISIONAL | `N-05-05` Claude Finding Enrichment |
...
| UC-EX03 | 기관이 추가 보완을 요구함 | 세무서 또는 UNKNOWN | 지원팀 담당자 또는 원문 지정 Actor | 보완 결과를 기록하고 기존 접수 유지 또는 재접수 여부를 판단한다. | 기관이 요구한 추가자료 | UNKNOWN | 기존 접수 유지 또는 재접수 판단 결과가 기록됨 | PROVISIONAL | `N-05-05` Claude Finding Enrichment |
| UC-EX04 | 기관 요구가 취하임 | 세무서 또는 UNKNOWN | 지원팀 담당자 또는 원문 지정 Actor | 취하 결과를 기록하고 취하 후 재접수 여부를 판단한다. | 취하 관련 기록 | UNKNOWN | 취하 종료 또는 재접수 판단 결과가 기록됨 | PROVISIONAL | `N-05-05` Claude Finding Enrichment |
...
- 보완 후 기존 접수 유지·재접수, 취하 후 재접수 여부의 세 하위 경로는 `PROVISIONAL`이다.
- 각 하위 경로의 감지조건·판단 Actor·정확한 복귀 Task·반복 횟수·처리기한은 `UNKNOWN`이며 자동 선택하지 않는다.
