...
| P3-C03 | Task `관련 조합` Rollup 실제값 | `TO DO LIST (FUND)` 원본 Rollup은 검증했으나 `지원팀 Task.상위 요청 → 지원팀 업무요청.관련 조합 → 지원팀 Task.관련 조합` 경로는 미검증 | N-06 `PARTIAL`, J-02 전 안전한 TEST 필수 | No | Yes |
...
## Verified but not sufficient for P3-C03 closure

- `TO DO LIST (FUND)`의 원본 Relation 저장
- 올바른 `전체관리조합` Record 연결
