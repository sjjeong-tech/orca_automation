const patterns = [
  { type: "RECEIPT", task: "P03-T04", words: ["접수증"] },
  { type: "RESULT_DOCUMENT", task: "P03-T05", words: ["고유번호증", "발급"] },
  { type: "STAMPED_DOCUMENT", task: "P03-T03", words: ["날인", "인감"] },
  { type: "SUBMISSION_PACKAGE", task: "P03-T02", words: ["신청서", "합본", "규약", "명부"] },
  ...
  { type: "BANK_DOCUMENT", task: null, words: ["거래신청서", "고객확인서", "금융거래목적확인서"] },
...
export function discoverEvidence({ files = [], fund_name = null, process_id = "P03", include_shortcuts = false, include_zero_byte = false, max_candidates = 100 } = {}) {
  ...
  for (const file of files) {
    ...
    if ((file.is_shortcut && !include_shortcuts) || (file.size === 0 && !include_zero_byte) || invalid) {
      invalid_candidates.push({ name: file.name, reason: file.size === 0 ? "ZERO_BYTE" : file.is_shortcut ? "INVALID_SHORTCUT" : fundMismatch(file, fund_name) ? "FUND_MISMATCH" : "SOURCE_PATH_MISSING" });
      continue;
    }
    const match = classify(file);
    if (!match) { unresolved_candidates.push({ name: file.name, reason: "UNCLASSIFIED_FILENAME" }); continue; }
    candidates.push({ name: file.name, path: file.path ?? null, evidence_type: match.type, task_candidate: match.task, validity: "CANDIDATE", human_confirmation_required: true, confirmation_questions: ["파일 본문·최신본·실물 또는 전달 여부를 확인했나요?"], completion_usable: false, modified_at: file.modified_at ?? null });
  }
