...
const staticHtml = fs.readFileSync("plugins/vc-support-admin/reports/console-pilot/request-task-console-v0.4.html", "utf8");
const outputSample = JSON.parse(fs.readFileSync("plugins/vc-support-admin/reports/console-pilot/request-task-console-v0.4-output.json", "utf8"));
const readinessArtifact = JSON.parse(fs.readFileSync("plugins/vc-support-admin/reports/console-pilot/request-task-console-v0.4-notion-readiness.json", "utf8"));
const demoGuide = fs.readFileSync("plugins/vc-support-admin/reports/console-pilot/request-task-console-v0.4-demo.md", "utf8");

assert.equal(CONSOLE_VERSION, "PILOT v0.4");
...
assert.match(htmlSource, /BACKEND_MODE=NOTION_LIVE_READ_PREVIEW/);
for (const panelId of ["request-inbox", "backend-status-panel", "request-panel", "current-action-panel", "kanban", "interaction-panel", "evidence-panel", "timeline", "execution-panel", "mapping-section", "relation-preview-panel", "duplicate-preview-panel", "property-mappings", "raw-json"]) {
...
assert.equal(outputSample.console_version, "PILOT v0.4");
...
assert.equal(readinessArtifact.notion_write_count, 0);
assert.equal(readinessArtifact.readiness.databases.fund_work.identifier_property, "조합명");
assert.doesNotMatch(staticHtml, /https?:\/\//i);
for (const text of ["REQ-DEMO-001", "REQ-DEMO-002", "REQ-DEMO-003", "DUMMY FUND RELATION", "TRANSACTION DUPLICATE", "WRITE NOT EXECUTED", "NOTION_WRITE_ENABLED=false"]) assert.ok(staticHtml.includes(text));
assert.match(demoGuide, /APPROVAL_BLOCKED/);
...
console.log("admin-process-console-v0.4: PASS");
